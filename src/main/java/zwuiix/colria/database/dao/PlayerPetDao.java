...
public interface PlayerPetDao {
    ...
    @SqlUpdate("CREATE INDEX IF NOT EXISTS idx_player_pets_identifier ON player_pets(identifier)")
    void initIndex();

    default void initAll() { init(); initIndex(); }

    @SqlUpdate("""
        INSERT INTO player_pets(xuid, identifier) VALUES(:xuid, :identifier)
    ...
    @SqlUpdate("DELETE FROM player_pets WHERE xuid = :xuid AND identifier = :identifier")
    void remove(@Bind("xuid") String xuid, @Bind("identifier") String identifier);

    @SqlUpdate("DELETE FROM player_pets WHERE xuid = :xuid")
    ...
    @SqlQuery("SELECT EXISTS(SELECT 1 FROM player_pets WHERE xuid = :xuid AND identifier = :identifier)")
    boolean has(@Bind("xuid") String xuid, @Bind("identifier") String identifier);

    @SqlQuery("SELECT identifier FROM player_pets WHERE xuid = :xuid ORDER BY identifier")
    List<String> listByXuid(@Bind("xuid") String xuid);

    @SqlQuery("SELECT xuid FROM player_pets WHERE identifier = :identifier ORDER BY xuid")
    List<String> listXuidsByIdentifier(@Bind("identifier") String identifier);

    @SqlQuery("SELECT COUNT(1) FROM player_pets WHERE xuid = :xuid")
