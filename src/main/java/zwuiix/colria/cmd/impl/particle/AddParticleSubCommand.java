...
public class AddParticleSubCommand extends ColriaPlayerSubCommand {
    ...
    private void processAsync(EnginePlayer player, String xuid, Particle particle, String targetName) {
        ...
        DataBase.getInstance().query(PlayerParticleDao.class, dao -> dao.has(xuid, particle.getIdentifier()))
                .whenCompleteAsync((has, err) -> {
                    ...
                    if(has) {
                        player.sendMessage(TranslationKeys.PLAYER_COMMAND_PARTICLE_ALREADYHAS, targetName, player.processTranslation(particle.getName()));
                        return;
                    }

                    DataBase.getInstance().write(PlayerParticleDao.class, (Consumer<PlayerParticleDao>) dao -> dao.add(xuid, particle.getIdentifier()));
                    ...
                    player.sendMessage(TranslationKeys.PLAYER_COMMAND_PARTICLE_ADDED, player.processTranslation(particle.getName()), targetName);
                });
    }
}
