...
public class GameUnFreezeSubCommand extends ColriaPlayerSubCommand {
    public GameUnFreezeSubCommand() {
        super("unfreeze");
    }
    ...
    public void execute(EnginePlayer player, Map<String, Object> args) {
        ...
        if(!game.getState().equals(Game.State.PAUSE)) {
            player.sendMessage(TranslationKeys.PLAYER_GAME_UNFREEZE_ALREADY);
            return;
        }

        game.setState(Game.State.RUNNING);
        game.broadcast(TranslationKeys.PLAYER_GAME_UNFREEZE_BROADCAST, player.getName());
        player.sendMessage(TranslationKeys.PLAYER_GAME_UNFREEZE_SUCCESS);
