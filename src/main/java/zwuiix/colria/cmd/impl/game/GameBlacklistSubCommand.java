...
public class GameBlacklistSubCommand extends ColriaPlayerSubCommand {
    public GameBlacklistSubCommand() {
        super("blacklist");
    }
    ...
    public void execute(EnginePlayer player, Map<String, Object> args) {
        ...
        switch(action) {
            case "add": {
                ...
                if(game.getBlacklist().contains(target)) {
                    player.sendMessage(TranslationKeys.PLAYER_GAME_BLACKLIST_ADD_ALREADY, target);
                    return;
                }

                game.getBlacklist().add(target);
                player.sendMessage(TranslationKeys.PLAYER_GAME_BLACKLIST_ADD_SUCCESS, target);
                return;
            }
            case "remove": {
                if(!args.containsKey("target")) throw new IllegalArgumentException("Missing target argument");
                ...
                String target = args.get("target").toString().toLowerCase();
                if(!game.getBlacklist().contains(target)) {
                    player.sendMessage(TranslationKeys.PLAYER_GAME_BLACKLIST_REMOVE_ALREADY, target);
                    return;
                }

                EnginePlayer targetPlayer = game.getSpectator(target);
                if(targetPlayer != null) targetPlayer.sudo("game leave");

                game.getBlacklist().remove(target);
                player.sendMessage(TranslationKeys.PLAYER_GAME_BLACKLIST_REMOVE_SUCCESS, target);
                return;
            }
            case "list": {
                if(game.getBlacklist().isEmpty()) {
                    player.sendMessage(TranslationKeys.PLAYER_GAME_BLACKLIST_LIST_EMPTY);
                    return;
                }

                player.sendMessage(TranslationKeys.PLAYER_GAME_BLACKLIST_LIST, String.join(", ", game.getBlacklist()));
            }
        }
    }
}
